...
namespace PdLogger.Net
{
	public abstract class ALoggerNetClient
	{
		...
		public void Connect(string source, int session)
		{
			...
			var constantData = new ConstantDataBuilder(source).Build(session);
			
			_pdNetEngine.OnResponse += (resp) =>
			{
				if (resp.GetHeader() == (byte)Header.Handshake)
				{
					var hr = (HandshakeResp)resp;
					if (hr.Success)
					{
					...
					{
						Debug.LogError("Handshake response error: " + hr.Error);
					}
				}
			};
