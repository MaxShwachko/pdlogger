...
namespace PdLogger.Impl
{
    public class Logger : MonoBehaviour, ILogger
    {
		...
		private void Start()
		{
			...
			_loggerNetClient.Connect(source);
			
			_sessionNumber = PlayerPrefs.GetInt(sessionSaveKey, 0);
			_sessionNumber++;
			PlayerPrefs.SetInt(sessionSaveKey, _sessionNumber);
			PlayerPrefs.Save();
		...
		private void LogCallback(string message, string stackTrace, LogType logType)
		{
			...
			builder.AddAdditionalField("session", _sessionNumber.ToString());
